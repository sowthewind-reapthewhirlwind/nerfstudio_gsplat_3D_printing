...
class SplatfactoModelConfig(ModelConfig):
    ...
    injected_means_file: str = "/mnt/c/Users/abhilash.durgam/Documents/ascc_parts/blender_and_background_means.npz"
...
class SplatfactoModel(Model):
    ...
    def populate_modules(self):
        ...
        if self.config.use_injected_means and (self.seed_points is None):
            if self.config.injected_means_file:
                print(f"Loading injected means from {self.config.injected_means_file}")
                injected = np.load(self.config.injected_means_file)
                # We expect the file to have a key "blender_means"
                injected_means_np = injected['blender_means']
                injected_means_tensor = torch.tensor(injected_means_np, dtype=torch.float32)
                # Since we're ignoring colors, create dummy colors (zeros)
                dummy_colors = torch.zeros(injected_means_tensor.shape[0], 3)
                self.seed_points = (injected_means_tensor, dummy_colors)
                print(f"Injected seed points shape: {self.seed_points[0].shape}")
            else:
    ...
    def step_post_backward(self, step):
        ...
        if self.step % 10000 == 0:
            ...
            save_path = f"/mnt/c/Users/abhilash.durgam/Documents/ascc_parts/model_means_{self.step}.npy"

            self._save_means_and_colors(save_path)
