...
class SplatfactoModelConfig(ModelConfig):
    ...
    use_injected_means: bool = True
    injected_means_file: str = "/mnt/c/Users/abhilash.durgam/Documents/ascc_parts/model_params_bottom_half.npz"
...
class SplatfactoModel(Model):
    ...
    def populate_modules(self):
        # Injection logic moved here, where self.config is guaranteed to be set.
        if self.config.use_injected_means and (self.seed_points is None):
            if self.config.injected_means_file:
                print(f"Loading injected parameters from {self.config.injected_means_file}")
                injected = np.load(self.config.injected_means_file)
                injected_means_tensor = torch.tensor(injected['means'], dtype=torch.float32)
                injected_scales_tensor = torch.tensor(injected['scales'], dtype=torch.float32)
                injected_quats_tensor = torch.tensor(injected['quats'], dtype=torch.float32)
                injected_features_dc = torch.tensor(injected['features_dc'], dtype=torch.float32)
                injected_features_rest = torch.tensor(injected['features_rest'], dtype=torch.float32)
                injected_opacities_tensor = torch.tensor(injected['opacities'], dtype=torch.float32)
                # You might not need colors if you're ignoring them.
                # Here, you can combine them into a dictionary or a tuple as you see fit.
                self.seed_points = (injected_means_tensor, injected_scales_tensor, injected_quats_tensor,
                                    injected_features_dc, injected_features_rest, injected_opacities_tensor)
                print(f"Injected means shape: {self.seed_points[0].shape}")
            else:
                print("No injected means file specified; falling back to default initialization.")
        
        # Now initialize the Gaussian means:
        if self.seed_points is not None and not self.config.random_init:
            means = torch.nn.Parameter(self.seed_points[0])
            scales = torch.nn.Parameter(self.seed_points[1])
            quats = torch.nn.Parameter(self.seed_points[2])
            features_dc = torch.nn.Parameter(self.seed_points[3])
            features_rest = torch.nn.Parameter(self.seed_points[4])
            opacities = torch.nn.Parameter(self.seed_points[5])
        ...
        # Since we're ignoring colors, initialize features_dc and features_rest with default values.
    ...
    def step_post_backward(self, step):
        ...
        if self.step % 10000 == 0:
            # You can incorporate the step number into the filename if you wish.
            save_path = f"/mnt/c/Users/abhilash.durgam/Documents/ascc_parts/model_params_{self.step}.npy"

            self._save_all_parameters(save_path)
    ...
    def _save_means(self, filepath: str) -> None:
        import os
        import numpy as np
        # Ensure the directory exists.
        os.makedirs(os.path.dirname(filepath), exist_ok=True)
        means_np = self.means.cpu().detach().numpy()
        np.save(filepath, means_np)
        print(f"Step {self.step}: Saved {means_np.shape[0]} means to {filepath}")

    def _save_means_and_colors(self, filepath: str) -> None:
        import os
        import numpy as np
        # Ensure the directory exists.
        os.makedirs(os.path.dirname(filepath), exist_ok=True)
        # Extract means and colors (features_dc is typically the base color)
        means_np = self.means.cpu().detach().numpy()
        colors_np = self.features_dc.cpu().detach().numpy()
        # Save both in a single .npz file
        np.savez(filepath, means=means_np, colors=colors_np)
        print(f"Step {self.step}: Saved {means_np.shape[0]} means and corresponding colors to {filepath}")


    def _save_all_parameters(self, filepath: str) -> None:
        import os
        import numpy as np
        # Ensure the directory exists.
        os.makedirs(os.path.dirname(filepath), exist_ok=True)
        # Extract all parameters from gauss_params as NumPy arrays.
        param_dict = {key: value.cpu().detach().numpy() for key, value in self.gauss_params.items()}
        np.savez(filepath, **param_dict)
        print(f"Step {self.step}: Saved {len(param_dict)} parameters to {filepath}")
